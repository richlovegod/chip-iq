...
`.github/workflows/daily.yml`：每個交易日兩個時段（台北 **18:17** 與 **20:47**，刻意避開整點）自動跑完上面全部腳本、跑三道回歸測試（分點、夥伴、全站結構）、commit 並發佈。任一道沒過就中止，不會把錯的數字推上去。也可以在 Actions 頁面手動觸發——**但避開台北 13:30–14:00**，TWSE 那段時間停用全市場查詢（自稱到 13:45，實測 13:53 仍擋，9/9 與 9/10 各撞一次）。所有腳本的 `date.today()` 以台北時間為準（workflow 設了 `TZ=Asia/Taipei`）。
